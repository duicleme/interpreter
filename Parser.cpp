...
auto parseVariable()->Variable* {
	...
	{
		cout <<" need for initialize a variable.";
		exit(1);
	}
...
auto parseOperand()->Expression *
{
	...
	switch ( current->kind )
	{
		...
		default:					cout << " bad expression.";		exit(1);
	}
...
auto skipCurrentIf(Kind token_kind)->bool {
	if (current->kind != token_kind ) return false;
	current++;
	return true;
}
